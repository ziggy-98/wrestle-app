...
export async function getAllForMatchesAndMerge(browser: Browser, matches: Match[], promotions: Promotion[]): Promise<Promotion[]>{
    ...
    for(const match of matches){
        ...
        const newPromotion = await getPromotionDetail(browser, match.promotion.url, match.promotion.name);
        promotionsToReturn.push(newPromotion);
    }
    return promotionsToReturn;
}

async function getPromotionDetail(browser: Browser, url: string, name: string): Promise<Promotion>{
    ...
    const statusEl = page.$(".Caption::-p-text(General Data) + .InformationBoxTable .InformationBoxRow .InformationBoxTitle::-p-text(Status) + .InformationBoxContents");
    const locationEl = page.$(".Caption::-p-text(General Data) + .InformationBoxTable .InformationBoxRow .InformationBoxTitle::-p-text(Location) + .InformationBoxContents");
    const activeEl = page.$(".Caption::-p-text(General Data) + .InformationBoxTable .InformationBoxRow .InformationBoxTitle::-p-text(Active Time) + .InformationBoxContents");
    ...
    for(const [elName, promiseResult] of Object.entries(elsMap)){
        ...
        if(elName === "location"){
            promotionToReturn.location = await value.evaluate((evalEl) => evalEl.textContent);
            continue;
        }
        if(elName === "active"){
